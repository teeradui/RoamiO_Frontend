...
function Label({ text, optional }: { text: string; optional?: boolean }) {
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginBottom: 6 }}>
      ...
      {optional && (
        <Text style={{ fontSize: 12, color: Colors.textMuted }}>(optional)</Text>
      )}
    </View>
  );
}
...
export default function EditTripScreen() {
  ...
  const { trips, fetchAllTrips, updateTrip, loading } = useTripController();
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      meetupTime: new Date(trip.meetUpTime),
      meetingPoint: trip.meetingPoint ?? "",
      profilePhoto: trip.imageUrl ?? null,
    });

    // Best-effort: parse "Country, State" back into picker
    const parts = trip.tripDestination.split(", ");
    if (parts.length >= 1) setCountry({ name: parts[0], flag: "", code: "" });
    if (parts.length >= 2) setSelectedState({ name: parts[1], isoCode: "" });
  }, [trips, id]);

  // ── Helpers (identical to create) ─────────────────────────────────────────
  ...
  const handleMapPress = async (e: MapPressEvent) => {
    ...
    if (result.length > 0) {
      ...
      const name = [place.name, place.district, place.city, place.country]
        .filter(Boolean)
        .join(", ");
  ...
  const selectPlace = (item: any) => {
    ...
    setFormData((prev: TripForm) => ({
      ...prev,
      meetingPoint: item.display_name.split(",").slice(0, 3).join(","),
    }));
    setSearchQuery("");
    setSearchResults([]);
  };
  ...
  const handleSave = async () => {
    ...
    if (updated) {
      ...
      Alert.alert("Error", "Failed to update the trip. Please try again.");
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.bgPrimary }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", marginTop: 16, marginBottom: 12, paddingHorizontal: 16 }}>
        ...
        <Text style={{ flex: 1, textAlign: "center", fontSize: 22, fontWeight: "700", color: Colors.textPrimary }}>
          Edit Trip
        </Text>
        <View style={{ width: 32 }} />
      </View>
      ...
      >
        ...
        <TouchableOpacity onPress={pickImage} style={{ alignItems: "center", marginBottom: 24 }}>
          ...
          <Text style={{ margin: 8, fontSize: 14, color: Colors.textSecondary, fontWeight: "500" }}>
            Upload Photo
          </Text>
        </TouchableOpacity>
        ...
        <View style={{ marginBottom: 16 }}>
          ...
          <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
            ...
            >
              <Text style={{ fontSize: 14, color: country ? Colors.textPrimary : Colors.textDisabled }}>
            ...
            <CountryPicker
              ...
              pickerButtonOnPress={(item) => {
                setCountry({ name: item.name.en, flag: item.flag, code: item.code });
            ...
            <TouchableOpacity
              onPress={() => { if (!country) return; setStateVisible(true); }}
              style={{
                ...inputStyle,
                flex: 1,
                flexDirection: "row",
            ...
            >
              <Text style={{ fontSize: 14, color: selectedState ? Colors.textPrimary : Colors.textDisabled, flex: 1 }} numberOfLines={1}>
        ...
        <View style={{ marginBottom: 24 }}>
          ...
          <TouchableOpacity
            ...
            style={{ ...inputStyle, marginBottom: 8, flexDirection: "row", alignItems: "center", gap: 8 }}
          >
            ...
            <View style={{ flex: 1, backgroundColor: "#00000060", justifyContent: "flex-end" }}>
              <View style={{ backgroundColor: Colors.bgCard, borderTopLeftRadius: 24, borderTopRightRadius: 24, height: "80%", overflow: "hidden" }}>
                    ...
                    <FlatList
                      ...
                      renderItem={({ item, index }) => (
                        <TouchableOpacity
                          ...
                          style={{ paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: index < searchResults.length - 1 ? 1 : 0, borderBottomColor: Colors.bgAccent }}
                        >
                ...
                <TouchableOpacity
                  ...
                  style={{ margin: 16, backgroundColor: Colors.btnPrimary, borderRadius: 30, paddingVertical: 14, alignItems: "center" }}
                >
        ...
        <TouchableOpacity
          ...
          style={{ backgroundColor: canSave ? Colors.btnPrimary : Colors.textDisabled, borderRadius: 30, paddingVertical: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }}
        >
          {loading ? (
            <ActivityIndicator color={Colors.bgPrimary} />
          ) : (
            <>
              <Text style={{ color: Colors.bgPrimary, fontSize: 16, fontWeight: "700" }}>Save</Text>
        ...
        <Modal visible={stateVisible} animationType="slide" transparent>
          <View style={{ flex: 1, backgroundColor: "#00000040", justifyContent: "flex-end" }}>
            <View style={{ backgroundColor: Colors.bgCard, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "60%", paddingTop: 16 }}>
              <Text style={{ textAlign: "center", fontSize: 16, fontWeight: "700", marginBottom: 12, color: Colors.textPrimary }}>
                Select State
              </Text>
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    onPress={() => { setSelectedState({ name: item.name, isoCode: item.isoCode }); setStateVisible(false); }}
                    style={{ paddingHorizontal: 24, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: Colors.bgAccent }}
                  >
              ...
              <TouchableOpacity
                ...
                style={{ margin: 16, padding: 14, borderRadius: 12, backgroundColor: Colors.bgAccent, alignItems: "center" }}
              >
